# Showcase

Page localisée (fr) — contenu aligné sur la documentation anglaise / espagnole pour **Showcase**.

## Entrée rapide

```bash
npx tsx packages/core/src/cliEntry.ts plan "your task"
npx tsx packages/core/src/cliEntry.ts run "your task" --stream
```

| Proyecto | Uso | Enlace |
|----------|-----|--------|
| *Tu proyecto* | *p. ej. auditoría CI + stream* | *URL* |


| Patrón | Docs |
|--------|------|
| Auditoría de seguridad | [Cookbook](/es/guide/cookbook/security-audit) |
| Refactor seguro | [Cookbook](/es/guide/cookbook/refactor-module) |
| CI full-auto | [Cookbook](/es/guide/cookbook/ci-full-auto) |


## Notes

- CLI monorepo : `packages/core/src/cliEntry.ts` · après build : `commander`  
- Métriques produit : 25 fournisseurs · 5 topologies · 18 tools · 6700+ tests  
- Pour le détail exhaustif, le monorepo et la version anglaise restent la source de vérité des signatures API  

## Lié

- [Vue d’architecture](/fr/architecture/overview)  
- [Démarrage rapide](/fr/guide/getting-started)  
- [Commandes](/fr/guide/commands)  
